from eliminacionGauss.models import Elim_Gauss, SmMrx
import json
...
class Matriz:
    ...
    def __init__(self, n, entradas=None, vista=None):
        """
        ...
        :param vista: nombre de la vista que llama a esta clase ('escalonar_process' o 'smMrx_process')
        """
        self.n = n  # Almacena el número de ecuaciones
        self.vista = vista  # Almacena la vista que llama a la clase

        # Dependiendo de la vista, carga los datos del modelo correspondiente
        if vista == 'escalonar_process':
            # Cargar datos del modelo Elim_Gauss
            matriz_data = Elim_Gauss.objects.first()  # Cambia esto según la lógica de tu aplicación
            self.matriz = self.obtener_matriz(matriz_data.EG_matriz)
        elif vista == 'smMrx_process':
            # Cargar datos del modelo SmMrx
            matriz_data = SmMrx.objects.first()  # Cambia esto según la lógica de tu aplicación
            self.matrices = self.cargar_matrices(matriz_data.sMrx_matrxS)
            self.escalar = matriz_data.sMrx_escalares  # Guardamos los escalares para su uso posterior
        else:
            raise ValueError("Vista no válida: debe ser 'escalonar_process' o 'smMrx_process'.")

    def cargar_matrices(self, json_data):
        """
        Carga las matrices desde un campo JSON y las convierte en una lista de matrices.
        :param json_data: datos en formato JSON que contienen las matrices.
        :return: lista de matrices convertidas a tipo flotante.
        :raises ValueError: Si alguna entrada no puede ser convertida.
        """
        try:
            matrices_json = json.loads(json_data)  # Decodifica el JSON
            return [self.obtener_matriz(matriz) for matriz in matrices_json]
        except (ValueError, json.JSONDecodeError) as e:
            raise ValueError("Error al cargar las matrices desde el JSON: " + str(e))
