document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!createMatrixInputsBtn || !matrixInputsContainer || !resolveBtn || !resolverForm || !resultText) {
    ...
    resolverForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            const datosAEnviar = JSON.stringify({
                cldt_Matrx: matriz // Convertir matriz a JSON
            });
    
            const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]').value;
    
            // Enviar datos al servidor usando Fetch, con el formato JSON
            const response = await fetch(resolverForm.action, {
                method: 'POST',
                body: datosAEnviar, // Usar datos en formato JSON
                headers: {
            ...
            if (data.status === 'success') {
                ...
                resultText.innerHTML = `
                    <strong>Matriz Inversa:</strong><br> ${data.resultado}
    ...
    function obtenerMatriz() {
        ...
        for (let i = 0; i < size; i++) {
            ...
            for (let j = 0; j < size; j++) {
                ...
                if (isNaN(value)) {
                    console.error(`Valor inválido en la posición (${i + 1}, ${j + 1}): ${input.value}`);
